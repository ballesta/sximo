...
class Sximo extends Model {
	...
	// Lis les lignes pour afficher la table
	//bb ajouté filtre pour club_id
	public static function getRows( $args )
	{
	   ...
	   $key = with(new static)->primaryKey;
        ...
        //bb 
		// Ajoute filtre pour le club pour les non administrateurs
		$group_id = \Session::get('gid');
		$club_id  = \Session::get('club_id', null);
		if ($group_id >=3)
		{
			// Pas administrateurs: accès aux données de leur club seulement 
		    $params .= " AND {$table}.club_id ='{$club_id}'"; 
		}
		//bb fin
    ...
    static function dd_ajax($valeur)
	{
	    $f = fopen("trace.html", "w");
		fclose($f);
	    $f = fopen("trace.html", "w");
		ob_start();
			var_dump($valeur);
    ...
    {   
		...
		{ 
			// Administrateur
			// voir tous les  clubs, pas de filtre
			$row =  \DB::table($table)->get();
		}
		else
		{
			// Non administrateur; filtrage par club
			$row =  \DB::table($table)->where('club_id',$club_id)->get();
		}
		$t['row'] = $row;
		self::dd_ajax($t);		
        return $row;
