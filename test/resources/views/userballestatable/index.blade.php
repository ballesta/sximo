...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="m-b row ">
		    <div class="col-md-6">
				@if($access['is_add'] ==1)
		   		<a href="{{ URL::to('userballestatable/update') }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
				...
				@endif 
				<a href="{{ URL::to( 'userballestatable/search') }}" class="btn btn-sm btn-white" onclick="SximoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="fa fa-search"></i> {{ Lang::get('core.btn_search') }} </a>				
	 ...
	 {!! Form::open(array('url'=>'userballestatable/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td width="100" class="text-right">
						...
						<a href="{{ URL::to('userballestatable/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('userballestatable/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("userballestatable/multisearch")}}');
		$('#SximoTable').submit();
	});
	...
	$('.copy').click(function() {
		...
		{
				$('#SximoTable').attr('action','{{ url("userballestatable/copy")}}');
